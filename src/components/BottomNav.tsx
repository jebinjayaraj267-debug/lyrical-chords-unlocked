...
import { Library, AudioLines, Mic, Guitar } from "lucide-react";
      ...
      <ul className="mx-auto flex max-w-lg items-stretch justify-between px-2 pb-[env(safe-area-inset-bottom)]">
        {items.map(({ to, label, Icon }) => (
          <li key={to} className="flex-1">
            <Link
              to={to}
              className="flex flex-col items-center gap-1 rounded-xl px-2 py-2.5 text-[11px] font-medium text-muted-foreground transition-colors"
